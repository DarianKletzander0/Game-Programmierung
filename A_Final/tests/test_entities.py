...
class PlayerTests(unittest.TestCase):
    ...
    def test_spread_shots_fan_out_from_one_stat(self) -> None:
        stats = PlayerStats(fire_cooldown=0.1, spread_shots=3)
        player = Player(pygame.Vector2(300, 600), stats)

        shots = player.update(0.11)

        self.assertEqual(len(shots), 3)
        self.assertEqual(len({round(shot.velocity.x, 2) for shot in shots}), 3)
        self.assertTrue(all(shot.velocity.y < 0 for shot in shots))

    def test_impulse_removes_only_nearby_hostile_projectiles(self) -> None:
        player = Player(pygame.Vector2(300, 600), PlayerStats(impulse_radius=30))
        near = Projectile(pygame.Vector2(320, 600), pygame.Vector2(), 5, True)
        far = Projectile(pygame.Vector2(340, 600), pygame.Vector2(), 5, True)
