...
class Renderer:
    ...
    def draw(self, surface: pygame.Surface, game) -> None:
        ...
        if game.state is ScreenState.SHOP:
            self._draw_shop_cards(surface, game)
            return
    ...
    def _draw_shop_cards(self, surface: pygame.Surface, game) -> None:
        self._text(surface, "UPGRADE-KNOTEN", self.large_font, WHITE, (SCREEN_WIDTH // 2, 70), center=True)
        self._text(
            surface,
            f"Sektor {game.sector_index + 1} stabilisiert  /  {game.run.currency} Datenfragmente verfuegbar",
            self.body_font,
            YELLOW,
            (SCREEN_WIDTH // 2, 116),
            center=True,
        ...
        rows = game.upgrades.rows()[:3]
        if not rows:
            self._text(
                surface,
                "ALLE UPGRADES MAXIMAL",
                self.heading_font,
                CYAN,
                (SCREEN_WIDTH // 2, 330),
                center=True,
            ...
            self._text(surface, "ENTER  /  NAECHSTEN SEKTOR STARTEN", self.heading_font, CYAN, (SCREEN_WIDTH // 2, 655), center=True)
            return

        key_labels = ["1", "2", "3"]
        for index, (name, item, level, cost) in enumerate(rows):
            rect = pygame.Rect(50 + index * 305, 175, 260, 255)
            self._panel(surface, rect)
            selected = game.upgrades.bought_offer == name
            already_bought = game.upgrades.bought_offer is not None and not selected
            affordable = cost is not None and game.run.currency >= cost and not already_bought
            color = GREEN if selected else CYAN if affordable else MUTED
            pygame.draw.rect(surface, color, rect, 2, border_radius=10)
            self._text(surface, key_labels[index], self.heading_font, color, (rect.x + 22, rect.y + 22))
            self._text(surface, item.label.upper(), self.heading_font, WHITE, (rect.x + 60, rect.y + 22))
            self._text(surface, item.rarity.upper(), self.tiny_font, color, (rect.x + 22, rect.y + 68))
            self._text(surface, item.description, self.small_font, MUTED, (rect.x + 22, rect.y + 96))
            price = "MAX" if cost is None else f"KOSTEN {cost}"
            self._text(surface, f"STUFE {level}/{item.max_level}", self.small_font, WHITE, (rect.x + 22, rect.y + 145))
            self._text(surface, price, self.small_font, color, (rect.x + 22, rect.y + 178))
            if selected:
                self._text(surface, "GEWAEHLT", self.small_font, GREEN, (rect.x + 22, rect.y + 213))
            elif already_bought:
                self._text(surface, "SHOP BEREITS GENUTZT", self.tiny_font, MUTED, (rect.x + 22, rect.y + 216))
